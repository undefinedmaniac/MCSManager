...
{
    const QString simpleString = simplifyString(data);

    if (simpleString == QStringLiteral("RESTART"))
        return Config::Restart;
    else if (simpleString == QStringLiteral("STARTALTSERVER"))
        return Config::StartAltServer;
